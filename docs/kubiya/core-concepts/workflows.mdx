...
<Tabs>
  <Tab title="Natural Language Input">
    ```text
    "Deploy the latest version of our payment service to staging, 
    run the integration tests, and if they pass, promote to 
    production with a gradual rollout"
    ```
  </Tab>
  
  <Tab title="AI-Generated Workflow">
    ```yaml
    name: payment-service-deployment
    description: "Safe deployment pipeline with automated testing"
    
    steps:
      - name: validate-prerequisites
        tool: deployment-validator
        checks:
          - staging_environment_healthy
          - no_blocking_incidents
          - deployment_window_active
      ...
      - name: deploy-to-staging
        ...
        inputs:
          ...
          environment: staging
          image: payment-service:${BUILD_VERSION}
...
```yaml
# AI automatically includes environment-specific details
steps:
  - name: scale-frontend
    tool: kubernetes-scaler
    # AI knows your cluster configuration
    cluster: "prod-us-west-2"
    namespace: "ecommerce"
    deployment: "frontend-service"
    # AI suggests scaling based on historical patterns  
    target_replicas: 15  # Usually 3x during flash sale events
    
  - name: warm-cache
    tool: redis-warmer
    # AI knows your cache topology
    redis_cluster: "prod-cache-cluster"
    # AI includes dependencies it discovered
    depends_on: [scale-frontend]
    
  - name: update-load-balancer
    tool: aws-alb-controller
    # AI knows your AWS configuration
    load_balancer: "prod-frontend-alb"
    # AI includes safety checks
