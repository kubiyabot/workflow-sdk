...
It enables you to list, search, describe, and even generate new tools with AI assistance, all with comprehensive error handling and flexible output formats.
...
- **Flexible Output**: Support for both dictionary and JSON output formats
...
All methods support flexible output formats:

```python
# Dictionary format (default)
tools_dict = client.tools.list()
print(f"Type: {type(tools_dict)}")  # <class 'list'>

# JSON string format
tools_json = client.tools.list(output_format="json")
print(f"Type: {type(tools_json)}")  # <class 'str'>
print(tools_json)  # Pretty-printed JSON

# Same applies to search and describe
search_json = client.tools.search("backup", output_format="json")
