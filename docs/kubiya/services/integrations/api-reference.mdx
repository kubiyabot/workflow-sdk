...
def safe_activate_integration(client, integration_name):
    ...
    try:
        ...
        print(f"ℹ️  Integration conflict: {e}")
...
Specialized exception for integration validation failures.
...
def setup_integration_with_retry(client, integration_name, max_retries=3):
    ...
    for attempt in range(max_retries):
        try:
            ...
            error_msg = str(e).lower()
            if "not supported" in error_msg:
                print("💥 Permanent error: integration not supported")
                return None
            elif "failed to create" in error_msg:
...
```python
def check_integration_status(client, integration_name):
    """Check if an integration is already active"""
    try:
        # Attempt activation to check status
        result = client.integrations.activate(integration_name)
        return {"status": "newly_activated", "result": result}
        
    except IntegrationNotFoundError as e:
        if "already exist" in str(e):
            return {"status": "already_active", "message": str(e)}
        else:
...
# Usage
status = check_integration_status(client, "github_app")
print(f"GitHub App integration status: {status['status']}")

if status["status"] == "already_active":
    print("✅ Integration is ready to use")
elif status["status"] == "newly_activated":
    print("🔧 Complete the installation process")
else:
    print(f"❌ Integration error: {status['message']}")
```
...
def setup_multiple_integrations(client, integrations):
    ...
    for integration_name in integrations:
        ...
        try:
            ...
            if "already exist" in str(e):
                ...
                print(f"ℹ️  {integration_name}: Already configured")
            else:
                results[integration_name] = {
                    "status": "error",
                    "message": str(e)
    ...
    for name, result in results.items():
        ...
        if status == "success":
            ...
            print(f"  ℹ️  {name}: Already configured")
        else:
