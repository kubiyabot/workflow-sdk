...
[
    {
        ...
        "formatted_title": "# 🔐 Authentication Guide",
        "icon": "🔐",
        "content": "# Authentication\n\nTo authenticate...",
        "raw_content": "To authenticate with Kubiya...",
        "is_code": False,
        "tags": ["auth", "security"],
        "metadata": {
            "title": "Authentication Guide",
            "icon": "🔐",
            "url": "https://docs.kubiya.ai/auth"
        }
    }
]
```
...
Specialized exception for when requested documentation cannot be found.
...
#### Attributes
- `message` (`str`): Error message indicating what documentation was not found
- `details` (`Dict[str, Any]`): Additional context about the failed search
...
try:
    ...
    if not results or all(r['score'] < 0.5 for r in results if 'score' in r):
        raise DocumentationNotFoundError(
            f"No relevant documentation found for query: 'very specific technical term'"
        )
    ...
    print("\n💡 Suggestions:")
    print("- Try broader search terms")
    print("- Check spelling and terminology")
    print("- Use common synonyms")
    print("- Break complex queries into simpler parts")
    
    # Suggest alternative searches
    original_query = "very specific technical term"
    suggestions = [
        original_query.split()[0],  # First word only
        "basic " + original_query.split()[-1],  # Last word with "basic"
        original_query.replace("technical", ""),  # Remove technical
    ]
