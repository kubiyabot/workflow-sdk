...
**API Version:**
- Uses `/api/v2/users` endpoint
- Supports v2 API response format with `items` wrapper
...
try:
    ...
    for user in users:
        ...
        # Access user roles if available
...
```python
# The service handles different API response formats
...
# You always get a consistent list format
...
**API Version:**
- Uses `/api/v1/manage/groups` endpoint
- V1 API format
...
**Response Format Handling:**
```python
# The service handles different response formats for groups
response = client.users.list_groups()

# The method automatically handles:
# - Direct list format: [...]
# - Wrapped format: {"groups": [...]}
# - Empty responses: [] (returns empty list)
