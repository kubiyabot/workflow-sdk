...
workflow = (
    ...
    .step("fetch-data", callback=lambda step:
        step.python("python fetch_data.py")
    ) # Container A is destroyed after execution

    # Step 2 runs in container B (completely separate)
    .step("process-data", callback=lambda step:
        step.python("python process_data.py")
    ) # Fresh container, no state from Step 1
)
```
...
workflow.step(
    name="ml-pipeline",
    callback=lambda step: step.docker(
        image="myregistry/ml-base:latest",
        command="# Use pre-installed ML tool",
    ),
)
```
...
workflow.step(
    ...
    callback=lambda step: step.docker(
        image="python:3.11",
        command="# Run memory-intensive operations"
    ),
    resources={
        "memory": "8Gi",
        "cpu": "4",
...
workflow.step(
    name="client",
    callback=lambda step: step.python(
        script="# Cannot access Step 1's ports directly"
    )
)
```
