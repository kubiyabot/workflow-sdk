...
wf = (
    ...
    # Build with Node.js
    .step("build", callback=lambda step:
        step.docker(
    ...
    .step("test", callback=lambda step:
        step.docker(
...
wf = (
    ...
    # Fetch logs
    .step("fetch-logs", callback=lambda step:
        step.docker(
    ...
    .step("analyze-logs", callback=lambda step:
        step.inline_agent(
...
""",
    ...
    # Generate report
    .step("generate-report", callback=lambda step:
        step.docker(
...
wf = (
    ...
    # Fetch PR diff
    .step("fetch-pr", callback=lambda step:
        step.docker(
    ...
    .step("ai-review", callback=lambda step:
        step.inline_agent(
...
""",
    ...
    # Post review comment
    .step("post-review", callback=lambda step:
        step.docker(
            image="alpine:latest",
            command=f"""
...
wf = (
    ...
    .step("detect-incident", callback=lambda step:
        step.docker(
        image="alpine:latest",
        command="""
    ...
    .step("ai-responder", callback=lambda step:
        step.inline_agent(
...
""",
    ...
    .step("execute-response", callback=lambda step:
        step.shell(
            """
    ...
    # Send notification
    .step("notify-team", callback=lambda step:
        step.docker(
            image="alpine:latest",
            command="""
            SEVERITY=$(echo "${RESPONSE_PLAN}" | jq -r '.severity')
...
wf = (
    ...
    .step("extract-db", callback=lambda step:
        step.docker(
            image="postgres:15",
            command="pg_dump -h ${DB_HOST} -U ${DB_USER} -d ${DB_NAME}"
        ).output("DB_DATA")
    )
    .step("extract-api", callback=lambda step:
        step.docker(
            image="python:3.11",
            command="""
                import requests
    ...
    .step("validate", callback=lambda step:
        step.docker(
            image="python:3.11",
            command="""
                import json
    ...
    # Load to warehouse
    .step("load", callback=lambda step:
        step.docker(
    ...
    # Error handling path
    .step("notify-error", callback=lambda step:
        step.inline_agent(
...
def generate_workflow():
    client = KubiyaClient(
        api_key="Your Kubiya API Key", # Replace with your actual API key
    )
